...
# dataset
data_dir = Path("data")
train = pd.read_csv(data_dir / "tt/kor_pair_train.csv").filter(
    items=["question1", "question2", "is_duplicate"]
)

test = pd.read_csv(data_dir / "tt/kor_pair_test.csv").filter(
    items=["question1", "question2", "is_duplicate"]
)
...
train.to_csv(data_dir / "tt/train.txt", sep="|", index=False)
validation.to_csv(data_dir / "tt/validation.txt", sep="|", index=False)
test.to_csv(data_dir / "tt/test.txt", sep="|", index=False)
